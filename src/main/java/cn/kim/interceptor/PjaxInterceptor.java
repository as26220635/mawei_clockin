...
public class PjaxInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //不是主界面url，就返回
        if (path.contains(LoginController.OAUTH_PATH) || path.contains(LayoutController.LAYOUT_PATH) || path.contains(MyHomeController.LAYOUT_PATH) || path.contains(ManagerController.LAYOUT_PATH) || path.contains("error/") || path.equals("/admin") || path.equals("/admin/")) {
            return true;
        }
