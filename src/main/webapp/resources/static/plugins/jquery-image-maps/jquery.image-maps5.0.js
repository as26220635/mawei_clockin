...
$.fn.imageMaps = function (setting) {
    ...
    function initAddClickEvent() {
        // 初始化添加热区功能按钮
        $("#" + setting.addBtn).unbind("click").click(function () {
            var index;
    ...
    function initDeleteClickEvent() {
        //初始化删除热区功能按钮
        $('.table #del').unbind('click').click(function () {
            var $this = $(this);
            setting.deleteCallBack($this.parents('tr'), ref);
            ...
            var index = 1;
            areaContainer.find('.table tr[ref]').each(function () {
                var $row = $(this);
                if ($row.attr('ref') != undefined) {
                    $(this).attr('ref', index);
                    // $(this).find('.areaSort').html('热区'+ index+"：");
                    setting.changeCallBack($(this), index);
                    index++;
                }
            });
            index = 1;
    ...
    this.each(function () {
        ...
        if (hotImage != null) {
            ...
            var imageContainer = hotImage.parent(), imgConrainerOffset = imageContainer.offset(),
        ...
        if ($('.hot_area img[ref=imageMap]').attr("src")
            ...
            if (areaInfoJson != null && areaInfoJson.length > 0) {
                for (var j = 0; j < areaInfoJson.length; j++) {
                    var id = areaInfoJson[j].id;
                    var index = areaInfoJson[j].index;
    ...
    function bindMapEvent() {
        ...
        $('.position_container .map_position .map_position_bg').each(function () {
            ...
            $(document).mousemove(function (event) {
                ...
                setting.changeAreaInfoCallBack(mapPosition.attr('ref'), Math.round(left), Math.round(top), Math.round(right), Math.round(bottom));
                return false;
...
imageMaps.proportionNoSameManual = function (pic, setting, proportionWidth, proportionHeight, imageMapsSwitch) {
    ...
    obj.on('load', datas, function (event) {
        //创建图片对象，设置src
        var img = new Image();
